...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS checkins (
            ...
            quest_name   TEXT,
            checkin_date TEXT,
            PRIMARY KEY (user_email, quest_name, checkin_date)
        )
    """)
    c.execute("""
...
def db_save_user(vorname, nachname, email):
    conn = get_conn()
    conn.execute("""
        INSERT INTO users (email, vorname, nachname) VALUES (?, ?, ?)
        ON CONFLICT(email) DO UPDATE SET vorname=excluded.vorname, nachname=excluded.nachname
    """, (email, vorname, nachname))
    conn.commit()
    conn.close()

def db_load_user(email):
    conn = get_conn()
...
def db_save_quest(quest, user_email, is_active=True):
    conn = get_conn()
    if is_active:
        conn.execute("UPDATE quests SET is_active = 0 WHERE user_email = ?", (user_email,))
    conn.execute("""
        INSERT INTO quests
            (user_email, name, start_datum, end_datum, ort, schwierigkeit, beschreibung, days_total, is_active)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        user_email,
        quest["name"],
        quest["start"].isoformat() if isinstance(quest["start"], date) else quest["start"],
        quest["end"].isoformat()   if isinstance(quest["end"],   date) else quest["end"],
        quest.get("ort"),
        quest.get("schwierigkeit"),
        quest.get("beschreibung"),
        quest.get("days_total"),
        1 if is_active else 0,
    ))
    conn.commit()
    conn.close()

def db_load_quests(user_email):
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM quests WHERE user_email = ? ORDER BY id DESC", (user_email,)
    ).fetchall()
    conn.close()
    result = []
    for row in rows:
        q = dict(row)
        q["start"] = date.fromisoformat(q["start_datum"])
        q["end"]   = date.fromisoformat(q["end_datum"])
        result.append(q)
    return result

def db_load_active_quest(user_email):
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM quests WHERE user_email = ? AND is_active = 1 LIMIT 1", (user_email,)
    ).fetchone()
    conn.close()
    if not row:
        return None
...
def db_save_checkin(user_email, quest_name):
    today = date.today().isoformat()
    conn = get_conn()
    conn.execute("""
        INSERT OR IGNORE INTO checkins (user_email, quest_name, checkin_date)
        VALUES (?, ?, ?)
    """, (user_email, quest_name, today))
    conn.commit()
    conn.close()

def db_is_checked_in_today(user_email, quest_name):
    today = date.today().isoformat()
    conn = get_conn()
    row = conn.execute("""
        SELECT 1 FROM checkins
        WHERE user_email = ? AND quest_name = ? AND checkin_date = ?
    """, (user_email, quest_name, today)).fetchone()
    conn.close()
    return row is not None

# ── DB: Archiv ────────────────────────────────────────────────────────────────

def db_save_to_archiv(quest, user_email, xp_bonus):
    conn = get_conn()
    conn.execute("""
        INSERT INTO archiv
            (user_email, name, start_datum, end_datum, ort, schwierigkeit, beschreibung, days_total, abgeschlossen_am, xp_bonus)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        user_email,
        quest["name"],
    ...
    conn.execute(
        "UPDATE quests SET is_active = 0 WHERE user_email = ? AND name = ?",
        (user_email, quest["name"])
    )
    conn.commit()
    conn.close()

def db_load_archiv(user_email):
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM archiv WHERE user_email = ? ORDER BY id DESC", (user_email,)
    ).fetchall()
    conn.close()
    result = []
    for row in rows:
...
def award_xp(amount, reason=""):
    ...
    if new_info["level"] > old_info["level"]:
        st.balloons()
        st.success(f"🎉 Level Up! Du bist jetzt **Level {new_info['level']} – {new_info['title']}**!")
    else:
    ...
    st.session_state.posts = [
        {
            ...
            "quest_name": "30 Tage Morgenroutine", "schwierigkeit": "Mittel",
            "quest_progress": 40, "is_friend": True, "same_challenge": True,
        },
        {
            "id": 2, "author": "Marco K.", "initials": "MK", "day": 12,
            "time": "vor 4 Std.",
            ...
            "img": None, "likes": 8, "liked": False,
            "comments": [{"author": "Lena M.", "text": "Kalte Dusche ist auf jeden Fall ein Gamechanger! 🥶"}],
            "quest_name": "30 Tage Morgenroutine", "schwierigkeit": "Mittel",
            "quest_progress": 40, "is_friend": True, "same_challenge": True,
        },
        {
            "id": 3, "author": "Jana T.", "initials": "JT", "day": 11,
            "time": "gestern",
            "text": "Morgenroutine komplett abgehakt! Yoga + Tagebuch schreiben. Diese Quest verändert wirklich meinen Alltag.",
            "img": None, "likes": 3, "liked": False, "comments": [],
            "quest_name": "30 Tage Morgenroutine", "schwierigkeit": "Mittel",
            "quest_progress": 37, "is_friend": False, "same_challenge": True,
        },
        {
            "id": 4, "author": "Felix R.", "initials": "FR", "day": 7,
            "time": "vor 1 Tag",
            "text": "Woche 1 geschafft! 🏋️ Jeden Tag ins Gym ist krasser als ich dachte, aber ich bin dabei.",
            "img": None, "likes": 12, "liked": False,
            "comments": [{"author": "Lena M.", "text": "Respekt! Weiter so 💪"}],
            "quest_name": "30 Tage Gym", "schwierigkeit": "Schwer",
            "quest_progress": 23, "is_friend": True, "same_challenge": False,
        },
        {
            "id": 5, "author": "Sophie W.", "initials": "SW", "day": 5,
            "time": "vor 2 Tagen",
            "text": "Heute 10 km gelaufen! Mein bisher weitester Lauf. Die Challenge motiviert mich wirklich.",
            "img": None, "likes": 6, "liked": False, "comments": [],
            "quest_name": "30 Tage Laufen", "schwierigkeit": "Schwer",
            "quest_progress": 17, "is_friend": False, "same_challenge": True,
        },
    ]

if "for_you_posts" not in st.session_state:
    st.session_state.for_you_posts = [{**p} for p in st.session_state.posts]
...
# Beim App-Start Daten aus DB laden wenn User eingeloggt
if st.session_state.user_email:
    email = st.session_state.user_email
    if not st.session_state.quests:
        st.session_state.quests = db_load_quests(email)
    if not st.session_state.active_quest:
        st.session_state.active_quest = db_load_active_quest(email)
    if not st.session_state.archiv:
        st.session_state.archiv = db_load_archiv(email)
...
def is_checked_in_today(quest_name):
    """Prüft zuerst DB (wenn eingeloggt), dann Session State als Fallback."""
    if st.session_state.user_email:
        return db_is_checked_in_today(st.session_state.user_email, quest_name)
    today_str = date.today().isoformat()
    return st.session_state.daily_checkin.get(quest_name) == today_str

def do_daily_checkin(quest_name, schwierigkeit):
    today_str = date.today().isoformat()
    ...
    if st.session_state.user_email:
        db_save_checkin(st.session_state.user_email, quest_name)
...
def page_account():
    ...
    # 1. PRÜFUNG: Ist der Nutzer eingeloggt?
    if st.session_state.user_email:
        user = db_load_user(st.session_state.user_email)
        
        # Header mit Logout-Button
        col_header, col_logout = st.columns([0.7, 0.3])
        with col_header:
            if user:
                st.success(f"Willkommen, **{user['vorname']}**!")
        with col_logout:
        ...
        # Sicherstellen, dass streak existiert (Fallback auf 0)
        ...
        if not quests:
            ...
            for q in quests:
                ...
                # Datumsberechnung (Sicherheitscheck ob q['start'] ein date-objekt ist)
                start_dt = q["start"]
                days_done = (date.today() - start_dt).days + 1
    ...
    # 2. LOGIN / REGISTRIERUNG (Wenn nicht eingeloggt)
    else:
        ...
        with tab_register:
            ...
            if submitted:
                if not zustimmung: st.error("Bitte AGB akzeptieren!")
                elif not email.strip() or not vorname.strip(): st.error("Pflichtfelder fehlen!")
                else:
                    db_save_user(vorname, nachname, email)
        ...
        with tab_login:
            with st.form("login_form"):
                ...
                login_btn = st.form_submit_button("Einloggen", type="primary")

            if login_btn:
                user = db_load_user(login_email.strip())
                if user:
...
def page_quest_erstellen():
    ...
    with st.form("quest_form"):
        ...
        with col3:
            ort = st.selectbox(
                ...
                index=None, placeholder="Ort wählen...",
            )
        with col4:
            schwierigkeit = st.selectbox(
                "Schwierigkeitslevel",
                ["Einfach", "Mittel", "Schwer", "Sehr Schwer"],
                index=None, placeholder="Schwierigkeit wählen...",
            )
    ...
    if submitted:
        ...
        if errors:
            ...
            new_quest = {
                "name": quest_name, "start": start_datum, "end": end_datum,
                "ort": ort, "schwierigkeit": schwierigkeit,
                "beschreibung": beschreibung, "days_total": days_total,
            }
...
def page_feed():
            ...
            st.markdown(
                ...
                <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);
                  ...
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <div style="background:#E23D5B;border-radius:20px;padding:4px 12px;
                                font-size:12px;font-weight:500">Aktiv</div>
                  </div>
    ...
    with st.form("post_form", clear_on_submit=True):
        post_text    = st.text_area("Was hast du heute geschafft?", placeholder="Beschreibe deinen Fortschritt...", height=100)
    ...
    if post_submitted:
        if not post_text.strip():
            ...
            schwierigkeit = (
                ...
                if st.session_state.active_quest else "Einfach"
            )
            new_post = {
...
def page_for_you():
    ...
    for post in posts:
        ...
        with st.container(border=True):
            ...
            with col_info:
                st.markdown(
                    f"<div style='margin-bottom:2px'><strong style='font-size:14px'>{post['author']}</strong>"
                    f" &nbsp;<span style='font-size:12px;color:gray'>{post['time']}</span></div>"
                    f"<div>{tag_badges}</div>", unsafe_allow_html=True,
                )
            st.markdown(
                f"""
                <div style="background:#fafafa;border-left:3px solid #E23D5B;border-radius:0 6px 6px 0;
                            padding:6px 10px;margin:8px 0;display:flex;justify-content:space-between;align-items:center">
                  <div>
            ...
            st.markdown(
                f"""<div style="height:4px;background:#f0f0f0;border-radius:2px;overflow:hidden;margin-bottom:10px">
            ...
            with col_like:
                ...
                if st.button(f"{heart} {post['likes']}", key=f"fy_like_{pid}"):
                    like_post(pid, "for_you"); st.rerun()
            with col_comment:
            ...
            if st.session_state.get(f"fy_show_comments_{pid}", False):
                ...
                with st.form(f"fy_comment_form_{pid}", clear_on_submit=True):
                    new_comment = st.text_input("Kommentar...", key=f"fy_cinput_{pid}", label_visibility="collapsed", placeholder="Schreib etwas Motivierendes... 💬")
                    if st.form_submit_button("Senden") and new_comment.strip():
                        add_comment(pid, new_comment.strip(), "for_you"); st.rerun()
    ...
    for q in reversed(archiv):
        ...
        st.markdown(
            ...
            <div style="background:linear-gradient(135deg,#f9fff5,#f0fbe8);
              ...
              <div style="position:absolute;top:14px;right:14px;background:#2ecc71;color:white;
                          border-radius:99px;font-size:11px;font-weight:600;padding:3px 10px">✅ Abgeschlossen</div>
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding-right:120px">
                <span style="font-size:16px;font-weight:600">{icon} {q['name']}</span>
                <span style="font-size:11px;padding:2px 8px;border-radius:99px;{badge_style}">{schwierigkeit}</span>
              </div>
              <div style="display:flex;flex-wrap:wrap;gap:10px;font-size:12px;color:#555;margin-bottom:10px">
                <span>📅 {start_str} – {end_str}</span>
                <span>⏱ {days_total} Tage</span>
                <span>📍 {ort}</span>
                <span>🏁 Abgeschlossen am {abschluss_str}</span>
              </div>
              <div style="background:rgba(46,204,113,0.12);border-radius:8px;padding:8px 12px;
                ...
                <div style="font-size:15px;font-weight:700;color:#2d7a4f">⭐ {xp_total} XP</div>
              </div>
            </div>
            """, unsafe_allow_html=True,
        )
...
def page_rangliste():
    ...
    leaderboard = [
        {"name": "Du",       "initials": "DU", "quests": len(st.session_state.quests), "xp": st.session_state.xp,  "level": get_level_info(st.session_state.xp)["level"]},
        {"name": "Lena M.",  "initials": "LM", "quests": 5,  "xp": 1250, "level": 4},
        {"name": "Marco K.", "initials": "MK", "quests": 3,  "xp": 800,  "level": 3},
        {"name": "Felix R.", "initials": "FR", "quests": 8,  "xp": 2100, "level": 6},
        {"name": "Sophie W.","initials": "SW", "quests": 4,  "xp": 950,  "level": 3},
    ]

    sort_key  = "quests" if st.session_state.lb_filter == "Quests" else "xp"
    lb_labels = {"quests": "Quests", "xp": "XP"}
    sorted_lb = sorted(leaderboard, key=lambda x: x[sort_key], reverse=True)
    max_val   = sorted_lb[0][sort_key] if sorted_lb else 1
    medals    = ["🥇", "🥈", "🥉"]
            ...
            <div style="border:1px solid {border_col};border-radius:10px;
              ...
              <div style="display:flex;align-items:center;gap:12px">
                ...
                <div style="text-align:right;flex-shrink:0">
                  <div style="font-size:14px;font-weight:500;color:#E23D5B">{entry[sort_key]} {lb_labels[sort_key]}</div>
                  <div style="font-size:11px;color:gray;margin-top:2px">{entry['quests']} Quests · {entry['xp']} XP</div>
                </div>
              </div>
            </div>
            """, unsafe_allow_html=True,
...
def explore_quest():
    ...
    EXPLORE_QUESTS = [
        {"id": "eq1", "name": "30 Tage Morgenroutine", "schwierigkeit": "Mittel",      "ort": "für zu Hause",     "days_total": 30, "beschreibung": "Stehe jeden Tag zur gleichen Zeit auf und starte mit einer festen Morgenroutine in den Tag – ob Meditation, Sport oder Journaling.", "teilnehmer": 1243, "emoji": "🌅"},
        {"id": "eq2", "name": "30 Tage Gym",            "schwierigkeit": "Schwer",      "ort": "im Fitnessstudio", "days_total": 30, "beschreibung": "Gehe 30 Tage in Folge ins Fitnessstudio. Kein Tag wird ausgelassen – Konsequenz ist der Schlüssel zum Erfolg.",                    "teilnehmer": 876,  "emoji": "🏋️"},
        {"id": "eq3", "name": "7 Tage Digital Detox",   "schwierigkeit": "Sehr Schwer", "ort": "für zu Hause",     "days_total": 7,  "beschreibung": "Verzichte eine Woche lang auf Social Media und nicht notwendige Bildschirmzeit.",                                                    "teilnehmer": 512,  "emoji": "📵"},
        {"id": "eq4", "name": "14 Tage täglich lesen",  "schwierigkeit": "Einfach",     "ort": "für zu Hause",     "days_total": 14, "beschreibung": "Lies jeden Tag mindestens 20 Minuten in einem Buch deiner Wahl. Bildung und Entspannung in einem.",                                 "teilnehmer": 2041, "emoji": "📖"},
        {"id": "eq5", "name": "21 Tage Laufen",          "schwierigkeit": "Mittel",      "ort": "im Freien",        "days_total": 21, "beschreibung": "Laufe 21 Tage lang mindestens 20 Minuten am Stück. Egal ob Regen oder Sonnenschein – du ziehst es durch!",                          "teilnehmer": 694,  "emoji": "🏃"},
        {"id": "eq6", "name": "30 Tage gesund ernähren","schwierigkeit": "Schwer",      "ort": "für zu Hause",     "days_total": 30, "beschreibung": "Koche jeden Tag selbst und verzichte auf Fast Food, Zucker und verarbeitete Lebensmittel.",                                           "teilnehmer": 1105, "emoji": "🥗"},
    ]
    ...
    if not remaining:
        st.markdown(
            """<div style="text-align:center;padding:3rem 1rem">
              ...
              <div style="font-size:14px;color:gray">Du hast alle verfügbaren Quests durchgesehen.</div>
            </div>""", unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        f"Quest {total_seen + 1} von {total_quests}</div>", unsafe_allow_html=True,
    )
    st.markdown(
        f"""
        <div style="background:#fff;border:1px solid #f0f0f0;border-radius:20px;
          ...
          <div style="text-align:center;margin-bottom:1.2rem">
            ...
            <div style="font-size:21px;font-weight:700;letter-spacing:-0.3px;margin-bottom:0.4rem">{quest['name']}</div>
            <span style="font-size:12px;padding:3px 10px;border-radius:99px;{badge_style}">{schwierigkeit}</span>
          </div>
          <div style="font-size:14px;color:#444;line-height:1.65;background:#fafafa;
                      border-radius:10px;padding:12px 14px;margin-bottom:1.2rem">{quest['beschreibung']}</div>
          <div style="display:flex;justify-content:space-around;text-align:center;margin-bottom:1.2rem">
            <div><div style="font-size:18px;font-weight:600;color:#E23D5B">{quest['days_total']}</div><div style="font-size:11px;color:gray">Tage</div></div>
            <div style="width:1px;background:#f0f0f0"></div>
            <div><div style="font-size:18px;font-weight:600;color:#E23D5B">+{total_xp}</div><div style="font-size:11px;color:gray">Max XP</div></div>
            <div style="width:1px;background:#f0f0f0"></div>
            <div><div style="font-size:18px;font-weight:600;color:#E23D5B">{quest['teilnehmer']:,}</div><div style="font-size:11px;color:gray">Teilnehmer</div></div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center">
            <span style="font-size:12px;background:#f5f5f5;border-radius:99px;padding:4px 12px;color:#555">{ort_icon} {quest['ort']}</span>
            <span style="font-size:12px;background:#FFF0F3;border-radius:99px;padding:4px 12px;color:#E23D5B">+{xp_day} XP/Tag</span>
            <span style="font-size:12px;background:#EAF3DE;border-radius:99px;padding:4px 12px;color:#3B6D11">+{xp_bonus} XP Bonus</span>
          </div>
        </div>
        """, unsafe_allow_html=True,
    )
    ...
    with col_accept:
        if st.button("✅ Annehmen", use_container_width=True, key="explore_accept", type="primary"):
            ...
            new_quest = {
                "name": quest["name"], "start": date.today(),
                "end":  date.today() + timedelta(days=quest["days_total"]),
                "ort":  quest["ort"], "schwierigkeit": quest["schwierigkeit"],
                "beschreibung": quest["beschreibung"], "days_total": quest["days_total"],
            }
            st.session_state.quests.append(new_quest)
            st.session_state.active_quest = new_quest
            if st.session_state.user_email:
    ...
    st.markdown(
        ...
        "❌ Ablehnen &nbsp;·&nbsp; ✅ Annehmen</div>", unsafe_allow_html=True,
    )
    if st.session_state.explore_accepted:
        st.markdown("<br>", unsafe_allow_html=True)
            ...
            for qid in st.session_state.explore_accepted:
                ...
                if match:
                    st.markdown(
                        ...
                        f"<span style='font-size:11px;color:gray'>{match['schwierigkeit']} · {match['days_total']} Tage</span>",
                        unsafe_allow_html=True,
                    )
...
if st.session_state.user_email:
    st.sidebar.markdown(
        f"<div style='font-size:12px;color:#2ecc71;margin-bottom:8px'>🟢 Eingeloggt als<br><strong>{st.session_state.user_email}</strong></div>",
        unsafe_allow_html=True,
    )
else:
    st.sidebar.markdown(
        "<div style='font-size:12px;color:#aaa;margin-bottom:8px'>⚪ Nicht eingeloggt</div>",
        unsafe_allow_html=True,
    )

nav_items = [
    ("👤 Account",           "account"),
    ("📤 Fortschritt teilen","feed"),
    ("✨ For You",           "for_you"),
...
st.sidebar.markdown(
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
      ...
      <div>
        <div style="font-size:13px;font-weight:500">Lv. {info['level']} – {info['title']}</div>
